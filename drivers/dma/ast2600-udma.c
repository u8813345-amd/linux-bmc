...
#define   UDMA_CH_CTRL_ADDRH	GENMASK(10, 8)
...
static void ast2600_udma_issue_pending(struct dma_chan *chan)
{
	...
	if (uc->is_tx) {
		ctrl = UDMA_BUFSZ_64KB |
		       FIELD_PREP(UDMA_CH_CTRL_ADDRH, upper_32_bits(ud->addr));

		writel(lower_32_bits(ud->addr), udma->regs + UDMA_CH_ADDR(ch_id));
		writel(ud->size, udma->regs + UDMA_CH_WPTR(ch_id));
		writel(ctrl, udma->regs + UDMA_CH_CTRL(ch_id));
	} else {
		/*
		...
		ctrl = FIELD_GET(UDMA_CH_CTRL_BUFSZ, ud->size) |
		       FIELD_PREP(UDMA_CH_CTRL_ADDRH, upper_32_bits(rx_addr));

		writel(lower_32_bits(rx_addr), udma->regs + UDMA_CH_ADDR(ch_id));
		writel(ctrl, udma->regs + UDMA_CH_CTRL(ch_id));
	}
...
static void ast2600_udma_remove(struct platform_device *pdev)
{
	struct device *dev = &pdev->dev;
